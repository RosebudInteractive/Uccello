        ...
        vDBNavigator.verHistButton = function () {
            ...
            if (db && rootElem) {
                ...
                /*for(var i=0 in tranList) {
                    ...
                    tranDiv.append('<p>'+i+' - guid:'+tranList[i].guid+', start:'+start+', end:'+end+', src:'+tranList[i].src+'</p>');
                ...
                var table = $('<table></table>'), thTable = $('<tr><th>version</th><th>guid</th><th>start</th><th>end</th><th>state</th><th>src</th><th>rootGuid</th><th>min</th><th>max</th></tr>');
                table.append(thTable);
               /* var index=0;
                for(var i in tranList) {
                    ...
                    var subTable = $('<table>');
                    if (index==0)
                    subTable.append('<tr><th>rootGuid</th><th>min</th><th>max</th></tr>');
                    index++;
                    for(var j in tranList[i].roots) {
                        var subTr = $('<tr></tr>');
                        subTr.append('<td>'+j+'</td>');
                        subTr.append('<td>'+tranList[i].roots[j].min+'</td>');
                        subTr.append('<td>'+tranList[i].roots[j].max+'</td>');
                        subTable.append(subTr);
                    }
                    var subTd = $('<td style="padding-top:0px; "></td>');
