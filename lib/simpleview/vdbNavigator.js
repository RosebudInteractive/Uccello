    ...
    function(template, tpl) {
        ...
        vDBNavigator.remoteCallsButton = function () {
            ...
            if (db && "getRcLog" in db) {
                ...
                tranDiv.empty()
                var table = $('<table><tr><th>time</th><th>type</th><th>trGuid</th><th>src</th><th>objGuid</th><th>func</th><th>aparams</th></table>'), thTable, that = this;
                for(var i in rcLog) {
                    var tr =$('<tr></tr>');
                    ...
                    time = time.getHours()+':'+time.getMinutes()+':'+time.getSeconds()+'.'+time.getMilliseconds();
                        ...
                        tr.append('<td>' + rcLog[i].src + '</td>');
                        tr.append('<td>' + rcLog[i].rc.objGuid + '</td>');
                        tr.append('<td>' + rcLog[i].rc.func + '</td>');

                        var params = $('<td class="params"><pre>' + (rcLog[i].rc.aparams?JSON.stringify(rcLog[i].rc.aparams, null, 1):'-') + '</pre></td>');
                        params.click(function(){
                            $(this).css('font-size', $(this).css('font-size')=='10px'?'14px':'10px');
                        });
                        tr.append(params);
                        table.append(tr);
                }
