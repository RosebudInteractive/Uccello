    ...
    function(template, tpl) {
        ...
        vDBNavigator.render = function (options) {
            ...
            if (db) {
                ...
                for (var i = 0; i < cnt; i++) {
                    ...
                    if (name == 'fc13e2b8-3600-b537-f9e5-654b7418c156')
                        name = 'metaroot';
                    ...
                    var link = leftTpl.find('a')
                        ...
                        .click(function () {
                            ...
                            $('.tabs ul:first li:eq(2) a').text("Версии рута: "+name);
                            return false;
                        });
        ...
        vDBNavigator.tranButton = function () {
            ...
            if (db && "getTranList" in db) {
                ...
                var table = $('<table></table>'), thTable = $('<tr><td colspan="10"><input type="button" value="truncate" /></td> </tr><tr><th>guid</th><th>start</th><th>end</th><th>state</th><th>src</th><th>rootGuid</th><th>min</th><th>max</th></tr>');
                ...
                for(var i=0, len=tranList.length; i<len; i++) {
                    ...
                    tr.append('<td><input type="checkbox" value="'+tranList[i].guid+'" class="checkTrans" id="ch'+tranList[i].guid+'"/> <label for="ch'+tranList[i].guid+'">'+tranList[i].guid+'</label></td>');
                    tr.append('<td>'+start+'</td>');
                    tr.append('<td>'+end+'</td>');
                    tr.append('<td>'+tranList[i].state+'</td>');
                    tr.append('<td>'+tranList[i].src+'</td>');
                    var index = 0;
                    for(var j in tranList[i].roots) {
                        if (index!=0) {
                            var tr = $('<tr></tr>');
                            tr.append('<td></td><td></td><td></td><td></td><td></td>');
                        }
                        tr.append('<td>'+j+'</td>');
                        tr.append('<td>'+tranList[i].roots[j].min+'</td>');
                        tr.append('<td>'+tranList[i].roots[j].max+'</td>');
                        table.append(tr);
                        index++;
                    }
        ...
        vDBNavigator.verHistButton = function () {
            ...
            if (db && rootElem) {
                ...
                var table = $('<table></table>'), thTable = $('<tr><th>version</th><th>guid</th><th>tran guid</th><th>start</th><th>end</th><th>state</th><th>src</th><th>rootGuid</th><th>min</th><th>max</th></tr>');
                table.append(thTable);
                for(var i in tranList) {
                    var tr =$('<tr></tr>');
                    var start = tranList[i].tr.start, end = tranList[i].tr.end?tranList[i].tr.end:null;
                    start = start.getHours()+':'+start.getMinutes()+':'+start.getSeconds()+'.'+start.getMilliseconds();
                    end = end?(end.getHours()+':'+end.getMinutes()+':'+end.getSeconds()+'.'+end.getMilliseconds()):'-';
                    tr.append('<td>'+tranList[i].ver+'</td>');
                    tr.append('<td>'+tranList[i].guid+'</td>');
                    tr.append('<td>'+tranList[i].tr.guid+'</td>');
                    tr.append('<td>'+start+'</td>');
                    tr.append('<td>'+end+'</td>');
                    tr.append('<td>'+tranList[i].tr.state+'</td>');
                    tr.append('<td>'+tranList[i].tr.src+'</td>');
                    var index = 0;
                    for(var j in tranList[i].tr.roots) {
                        if (index!=0) {
                            var tr = $('<tr></tr>');
                            tr.append('<td></td><td></td><td></td><td></td><td></td><td></td><td></td>');
                        }
                        tr.append('<td>'+j+'</td>');
                        tr.append('<td>'+tranList[i].tr.roots[j].min+'</td>');
                        tr.append('<td>'+tranList[i].tr.roots[j].max+'</td>');
                        table.append(tr);
                        index++;
                    }
                    if (index == 0) {
