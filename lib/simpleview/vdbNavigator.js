    ...
    function(template, tpl) {
        ...
        vDBNavigator.viewRight = function (obj) {
            ...
            if (obj.count) {
                ...
                right.append('<p><span class="name" style="width: 100%;vertical-align: top;">'+('getRootVersion' in obj? 'Root versions valid:'+obj.getRootVersion('valid')+' sent:'+obj.getRootVersion('sent')+' draft:'+obj.getRootVersion('draft'): '')+'</span></p>');
                for (var i = 0, len = obj.count(); i < len; i++) {
                    if (obj.getFieldType) {
